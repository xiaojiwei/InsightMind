...
为什么这很重要？因为 LLM 在数据分析领域能不能正常工作，关键不在于模型本身有多强——而在于它拿到的 context 够不够清晰。表名 `ods_sales_fact_v2` 对 LLM 来说毫无意义，但"销售额，按日期、渠道、区域可拆解，口径为 `SUM(price × quantity)`"才是它能理解的语言。**InsightMind 做的，就是把前者自动翻译成后者。**
