...
from demo_call_sop_data import DEMO_STORE, DEMO_WEEK_START, DEMO_DAY, build_demo_records
...
def test_demo_call_sop_dataset_is_complete_and_sanitized():
    ...
    assert len(rows) == 54
    assert sum(1 for row in rows if row["connected"]) == 52
    assert len({row["activity_date"] for row in rows}) == 7
    assert min(row["activity_date"] for row in rows) == DEMO_WEEK_START
    assert max(row["activity_date"] for row in rows) == DEMO_DAY
    assert {row["expert_name"] for row in rows} == {
        "王宇航",
        "赵明轩",
        "林若琪",
        "陈思源",
...
def test_demo_call_sop_dataset_triggers_monitoring_scenarios():
    ...
    assert all(
        sum(1 for row in rows if row["expert_name"] == name) >= 5
        for name in {row["expert_name"] for row in rows}
    )
