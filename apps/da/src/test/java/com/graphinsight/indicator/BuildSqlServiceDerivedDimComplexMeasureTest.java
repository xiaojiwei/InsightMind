 ...
 */
public class BuildSqlServiceDerivedDimComplexMeasureTest {
...
//        factTable1.setWhereCondition("gender=1");
        ...
        /**
         * 派生指标（复合指标）
         *  measure6 = measure1 + measure2
         */
        Measure measure4 = new Measure();
        measure4.setCode("meas_6");
        measure4.setName("派生复合指标6");
        measure4.setAlias("派生复合指标6");
        ...
        OperationItem pitem0 = new OperationItem();
        pitem0.setOperatingType("operator");
        pitem0.setOperator("least");

        itemList1.add(pitem0);
        ...
        pitem2.setOperatingType("operand");
        OperationItem.MeasureBasicInfo pBasicInfo = new OperationItem.MeasureBasicInfo(Long.valueOf(1), measure1.getCode(), measure1.getName());
        pitem2.setOperand(pBasicInfo);

        itemList1.add(pitem2);

        OperationItem pitem3 = new OperationItem();
        pitem3.setOperatingType("operator");
        pitem3.setOperator(",");

        itemList1.add(pitem3);


        OperationItem pitem4 = new OperationItem();
        pitem4.setOperatingType("operand");
        OperationItem.MeasureBasicInfo pBasicInfo1 = new OperationItem.MeasureBasicInfo(Long.valueOf(3), measure3.getCode(), measure3.getName());
        pitem4.setOperand(pBasicInfo1);

        itemList1.add(pitem4);

        OperationItem pitem5 = new OperationItem();
        pitem5.setOperatingType("operator");
        pitem5.setOperator(")");

        itemList1.add(pitem5);

        measure4.setExpList(itemList1);
...
//        dimension2.setMaster(true);
        ...
        dimension3.setName("衍生维度");
        dimension3.setAlias("衍生维度");
        ...
        operatoreq.setSqlOprType(SqlOprType.DERIVED_EQUAL_ID);
...
//        orderList.add(measOrder);
        ...
        List filterTreeList = new ArrayList<>();
        FilterTree filterTree = new FilterTree();


        Filter dimFilter6 = new Filter();
        dimFilter6.setCode("dim_3");
        Operator operator6 = new Operator();
        operator6.setSqlOprType(SqlOprType.LIKE);
        operator6.getDataList().add("值1");

        dimFilter6.getOperatorList().add(operator6);

        filterTree.setFilterType(FilterType.FILTER);
        filterTree.setFilter(dimFilter6);
        filterTree.setSqlLogicalType(SqlLogicalType.AND);
        filterTree.setCode("dim_3");

        filterTreeList.add(filterTree);

        queryParam.setFilterTreeList(filterTreeList);
        ...
        if (!isMeasureOpr) {
             ...
             * 需要制作同环比连接
             */
            boolean isOpenRatio = mockBuildSqlService.checkOpenRadioInfo(tuple);
            if (isOpenRatio) {
                String radioSql = mockBuildSqlService.buildRadioSql(tuple, aggregatorSql);
            }
            ...
            String sql = QueryExecutorService.formatSql(SourceType.MYSQL, hasMeasOrderPageSql);
