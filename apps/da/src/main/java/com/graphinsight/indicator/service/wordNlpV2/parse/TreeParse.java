package com.graphinsight.indicator.service.wordNlpV2.parse;
...
public class TreeParse {
    ...
    public TextNodeVo textParse(String wordText) {
        // 调用接口解析文本树
        WordSyntaxVo wordSyntaxVo = wordSyntax.splitInfo(wordText);
        ...
        TextNodeVo textNodeTree = getTextNode(wordText, wordSyntaxVo);
        //
        traverse(textNodeTree, "", mapWord, wordSyntaxVo);
        log.info("mapWord is:{}", mapWord);

        return textNodeTree;
    }

    public void traverse(TextNodeVo node, String prefix, Map<String, WordSyntaxVo.WordItem> mapWord, WordSyntaxVo wordSyntaxVo) {
        TextNodeVo root = node;
        if (node == null) return;
        for (TextNodeVo childNode : node.children) {
            // 判断是否是子条件
            if (null != mapWord.get(childNode.getWordName())) {
                WordSyntaxVo.WordItem wordItem = mapWord.get(childNode.getWordName());
                if (wordItem.getSqlType() != null && wordItem.getSqlType().equals("where")) {
                    // 如果深度大于1，说明是子条件
                    if (childNode.getDeep() > 1) {
                        // 获取最近的父级节点，并且最近的父级节点类型是指标的话，则添加到该节点下；
                        // 如果只有一个指标，则直接添加到该节点下
                        // 如果是多个指标，没有找到临近的，直接升级为全局条件
                        Result result = findNodeAndRelativesHelper(root, childNode.getWordName(), mapWord, null);
                        TextNodeVo parentNode = null;
                        if (result.parent != null) {
                            parentNode = result.parent;
                            log.info("Parent Node: " + result.parent);
                        } else if (result.sibling != null) {
                            parentNode = result.sibling;
                            log.info("Sibling Node: " + result.sibling);
                        }
                        if (parentNode != null) {
                            // 顺序识别，前面的条件是分子，后面的条件是分母
                            // 如果条件与分子条件并行，作为分子条件
                            if (wordSyntaxVo.getNumerator().getUniqueKey() == null || wordSyntaxVo.getNumerator().getUniqueKey().equals(parentNode.getWordIndex() + '~' + parentNode.getWordName())) {
                                wordItem.setSqlType("numeratorWhere");
                                wordSyntaxVo.getNumerator().setUniqueKey(parentNode.getWordIndex() + '~' + parentNode.getWordName());
                                wordSyntaxVo.getNumerator().getSonTextList().add(childNode.getWordName());
                            } else {
                                if (wordSyntaxVo.getDenominator().getUniqueKey() == null || wordSyntaxVo.getDenominator().getUniqueKey().equals(parentNode.getWordIndex() + '~' + parentNode.getWordName())) {
                                    wordItem.setSqlType("denominatorWhere");
                                    wordSyntaxVo.getDenominator().setUniqueKey(parentNode.getWordIndex() + '~' + parentNode.getWordName());
                                    wordSyntaxVo.getDenominator().getSonTextList().add(childNode.getWordName());
                                }
                            }
                        } else {
                            // 没有找到父级指标节点，则直接添加到全局条件
                            // 全局条件暂时不做处理
                            log.info("没有找到临近的父级或者兄弟指标节点");
                        }
                    }
                }
            }

            traverse(childNode, prefix + "--", mapWord, wordSyntaxVo);
        }
    }
    ...
    static class Result {
        ...
        Result(TextNodeVo parent, TextNodeVo sibling) {
            this.parent = parent;
            this.sibling = sibling;
        }
    }

    static Result findNodeAndRelativesHelper(TextNodeVo node, String targetName, Map<String, WordSyntaxVo.WordItem> mapWord, TextNodeVo parent) {
        if (Objects.equals(node.getWordName(), targetName)) {
            // 找到目标节点，检查其父节点是否符合条件
            if (parent != null && null != mapWord.get(parent.getWordName()) && mapWord.get(parent.getWordName()).getMatchType() != null && mapWord.get(parent.getWordName()).getMatchType().equals("measure")) {
                return new Result(parent, null);
            } else {
                // 查找兄弟节点中是否有符合条件的节点
                if (parent != null) {
                    for (TextNodeVo sibling : parent.children) {
                        if (!Objects.equals(sibling.getWordName(), targetName) && null != mapWord.get(sibling.getWordName()) && mapWord.get(sibling.getWordName()).getMatchType() != null && mapWord.get(sibling.getWordName()).getMatchType().equals("measure")) {
                            return new Result(null, sibling);
                        }
                    }
                }
        ...
        for (TextNodeVo child : node.children) {
            Result result = findNodeAndRelativesHelper(child, targetName, mapWord, node);
            if (result.parent != null || result.sibling != null) {
                return result;
            }
        }
        return new Result(null, null);
    ...
    public TextNodeVo getTextNode(String wordText, WordSyntaxVo wordSyntaxVo) {
        ...
        String url = "https://da-nlp-dev.inner.chj.cloud/hanlp/dep?text=" + wordText;
    ...
    public Map<String, List<TextNodeVo>> textParseTest(String wordText) {
