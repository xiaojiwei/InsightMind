...
@Component
@Slf4j
public class SubConCompareChain extends AbstractWordChain {
    ...
    public WordSyntaxVo textParse(WordSyntaxVo wordSyntaxVo) {
        ...
        String currentDatePatter = "(同比|环比|同环比)";
        ...
        for (int i = 0; i < wordSyntaxVo.getWordItemList().size(); i++) {
            ...
            if (Objects.equals(wordItem.getMatchType(), "date")
                    && wordItem.getValueList().size() >= 2
                    && !Objects.equals(wordItem.getValueList().get(0), wordItem.getValueList().get(1))) {
                dateMap = wordItem.getValueMap();
                changeIndex = i;
                isSubCompare = true;
                break;
            }
        ...
        List<List<Map<Integer, String>>> sortedList = findAndRecordNodesInfo(textNodeTree);
        log.info("sortedList: {}", sortedList);

        Map<String, String> dateTextMap = new LinkedHashMap<>();
        // 先查出两个日期的句子
        // 在根据两个日期的句子，查找出共同的指标
        Integer dateShowNum = 1;
        String subDate = "";
        String allDate = "";
        Boolean flag = false;

        Integer rationIndex = 0;
        for (int i = 0; i < sortedList.size(); i++) {
            List<Map<Integer, String>> textListMap = sortedList.get(i);
            for (int j = 0; j < textListMap.size(); j++) {
                Map<Integer, String> textMap = textListMap.get(j);
                for (Integer key : textMap.keySet()) {
                    if (textMap.get(key).contains("环比") || textMap.get(key).contains("同比")) {
                        rationIndex = i;
                        flag = true;
                        break;
                    }
        ...
        if (flag && rationIndex > 0) {
            for (int k = rationIndex; k > 0; k--) {
                List<Map<Integer, String>> textListMap = sortedList.get(k);
                for (int j = 0; j < textListMap.size(); j++) {
                    Map<Integer, String> textMap = textListMap.get(j);
                    for (Map.Entry<Integer, String> entry : textMap.entrySet()) {
                        if (null != dateMap.get(entry.getValue())) {
                            breakFlag = true;
                            subDate = dateMap.get(entry.getValue());
                            wordSyntaxVo.setNatureRatioFlag(true);
                            wordSyntaxVo.getCompareRatio().setSubDate(subDate);
                            wordSyntaxVo.getCompareRatio().setUniqueKey(subDate);
                            break;
                        }
                    }
                    if (breakFlag) {
        ...
        if (isSubCompare && breakFlag && null != wordSyntaxVo.getCompareRatio().getSubDate()) {

            wordSyntaxVo.getWordItemList().get(changeIndex).getValueList().remove(wordSyntaxVo.getCompareRatio().getSubDate());

            if (wordSyntaxVo.getWordItemList().get(changeIndex).getValueList().size() == 1) {
                wordSyntaxVo.getWordItemList().get(changeIndex).getValueList().add(wordSyntaxVo.getWordItemList().get(changeIndex).getValueList().get(0));
            } else {
                List<String> dateList = new ArrayList<>();
                dateList.add(wordSyntaxVo.getWordItemList().get(changeIndex).getValueList().get(0));
                dateList.add(wordSyntaxVo.getWordItemList().get(changeIndex).getValueList().get(wordSyntaxVo.getWordItemList().get(changeIndex).getValueList().size() - 1));
                wordSyntaxVo.getWordItemList().get(changeIndex).setValueList(dateList);
            }
        }
    ...
    public static List<List<Map<Integer, String>>> findAndRecordNodesInfo(TextNodeVo root) {
        Map<String, List<TextNodeVo>> subMap = new LinkedHashMap<>();


        Set<String> visited = new HashSet<>();

        List<List<Map<Integer, String>>> sortedList = new ArrayList<>();
        if (root != null) {
            // 先访问根节点
            for (TextNodeVo child : root.children) {
                List<Map<Integer, String>> itemList = new ArrayList<>();
                preOrderTraversal(child, itemList, visited);
                // 遍历每个子节点
    ...
    private static void preOrderTraversal(TextNodeVo node, List<Map<Integer, String>> itemList, Set<String> visited) {
        if (node == null || visited.contains(node.getWordName() + "~" + node.getWordIndex())) {
            return;
        }

        Map<Integer, String> itemMap = new HashMap<>();
        itemMap.put(node.getWordIndex(), node.getWordName());
